...
def preprocess_data(filepath):
    ...
    # Save processed data
    np.save("X_tech_final.npy", X_final)
    print("Saved processed matrix to X_tech_final.npy")
    
    # Save keywords for later analysis
    with open("extracted_keywords_tech.txt", 'w') as f:
        f.write("Top Keywords from Transaction Descriptions:\\n")
        f.write("=" * 50 + "\\n")
        for i, word in enumerate(keywords[:50], 1):
            f.write(f"{i}. {word}\\n")
    print("Saved keywords to extracted_keywords_tech.txt")
    
    return X_final

if __name__ == "__main__":
    preprocess_data("tech_transactions_enriched.csv")
