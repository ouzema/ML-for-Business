...
def load_and_filter_data(filepath):
    """Loads data, filters for Eurozone TECH companies."""
    print("Loading data...")
    ...
    # Filter for TECH companies based on Transaction Comments and company names
    print("Filtering for Technology sector...")
    tech_keywords = [
        'technology', 'software', 'tech', 'digital', 'cloud', 'saas', 'platform',
        'internet', 'mobile', 'app', 'data', 'analytics', 'ai', 'artificial intelligence',
        'machine learning', 'cybersecurity', 'fintech', 'e-commerce', 'online',
        'semiconductor', 'chip', 'hardware', 'electronics', 'telecom', 'it services',
        'computer', 'automation', 'robotics', 'blockchain', 'crypto'
    ]
    
    def is_tech_company(row):
        """Check if company is tech-related based on available data."""
        text_to_check = []
        
        # Check Transaction Comments
        if pd.notna(row.get('Transaction Comments')):
            text_to_check.append(str(row['Transaction Comments']).lower())
        
        # Check Target/Issuer name
        if pd.notna(row.get('Target/Issuer')):
            text_to_check.append(str(row['Target/Issuer']).lower())
        
        # Check Acquiror name
        if pd.notna(row.get('Acquiror/Investor(s)')):
            text_to_check.append(str(row['Acquiror/Investor(s)']).lower())
        
        combined_text = ' '.join(text_to_check)
        
        # Check if any tech keyword appears
        return any(keyword in combined_text for keyword in tech_keywords)
    
    df_tech = df_euro[df_euro.apply(is_tech_company, axis=1)].copy()
    
    print(f"Technology sector transactions: {len(df_tech)}")
    print(f"Using all {len(df_tech)} Tech transactions for analysis.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        df.to_csv("tech_transactions.csv", index=False)
        print("Saved filtered tech data to tech_transactions.csv")
        
        # Enrich
        df_enriched = enrich_data(df)
        df_enriched.to_csv("tech_transactions_enriched.csv", index=False)
        print("Saved enriched tech data to tech_transactions_enriched.csv")
