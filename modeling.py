...
def run_modeling(data_path, original_data_path):
    ...
    for k in [3, 4]:
        ...
        if X_pca.shape[0] > 2000:
            ...
            plt.savefig(f"clusters_visualization_k{k}.png", dpi=150, bbox_inches='tight')
            plt.close()
        else:
            tsne = TSNE(n_components=2, random_state=42, perplexity=30)
            ...
            plt.savefig(f"clusters_visualization_k{k}.png", dpi=150, bbox_inches='tight')
            plt.close()
        print(f"✓ Saved visualization to clusters_visualization_k{k}.png")
        
        # Save Clustered Data
        df_clustered = df_orig.copy()
        df_clustered['Cluster'] = cluster_labels
        output_file = f"eurozone_transactions_clustered_k{k}.csv"
    ...
    df_clustered = df_orig.copy()
    df_clustered['Cluster'] = cluster_labels
    df_clustered.to_csv("eurozone_transactions_clustered.csv", index=False)
    print(f"✓ Saved default clustered data (K=3) to eurozone_transactions_clustered.csv")
...
if __name__ == "__main__":
    run_modeling("X_final.npy", "eurozone_transactions_enriched.csv")
